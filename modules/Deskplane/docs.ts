...
export const GridSVGDemo: act.Component = () => {
  ...
  useAnimation("GridSVGAnim", ({ now }) => {
    patternRef.current?.setAttribute(
      "x",
      (Math.cos(now / 1000) * 100).toString()
    );
    patternRef.current?.setAttribute(
      "y",
      (Math.sin(now / 1000) * 100).toString()
    );
  });
...
export const DeskPlaneDemo = () => {
  ...
  return h(FramePresenter, {}, [
    h(
      Deskplane,
      { overlayChildren: [h("h2", { style: overlayStyle }, "Hello")] },
      [
...
export const DraggableSurfaceDemo = () => {
  ...
  const onDrag = useMemo(
    () => (delta: three.Vector2, element: Element) => {
      dragged.add(delta);
